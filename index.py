...
def cal():
    ...
    if request.method == "POST" and  request.files["direct"].filename!="" and  request.files["indirect"].filename!="" :
        ...
        ddata = pd.read_csv('C:/upload/direct/'+d.filename+'',skiprows=-1,low_memory=False)
        dli=ddata.values.tolist()

        # for indirect data to making list of csv data
        idata = pd.read_csv('C:/upload/indirect/'+i.filename+'',skiprows=-1,low_memory=False)
...
def poInsert(row,col,pso):
    ...
    # cur = mysql.connection.cursor()
    # print(direct)
    # for i in range(0,len(direct[:22])):
    #     cur.execute("insert into direct(sr,name,cie1,ca1,subject,course,college) values(%s,%s,%s,%s,%s,%s,%s)",(direct[i][0],direct[i][1],direct[i][2],direct[i][3],subject,str(course),college))
...
def subadmin():
    ...
    # important for subject
            ...
            cur.execute("delete from direct where cie1='0' or ca1='0' and course=%s",(course,))
...
def nobita():
    ...
    if 'subs' in session and 'subadmin' in session:
        ...
        if session['yearop'] == "year":
            nyear=(year+int(session["year"])-int(session['selectedYear']))
            for h in range(0,len(subject_name)):
                direct=cur.execute("select * from direct where subject=%s and course=%s and year=%s and college=%s",(str(subject_name[h][4]),str(course),nyear,clg,))
                direct=cur.fetchall()
                directdata.append(direct)
                indirect=cur.execute("select * from indirect where subject=%s and course=%s and year=%s and college=%s",(str(subject_name[h][4]),str(course),nyear,clg,))
                indirect=cur.fetchall()
                indirectdata.append(indirect)
                # pe11=cur.execute("select po.val from po where subject='"+str(subjects[h][4])+"' and course='"+str(course)+"' and year=%s and college=%s",(nyear,clg,))
                # pe11=cur.fetchall()
                # poe11 = list((int(x[0])) for x in pe11)
                # resultTuple = list(poe11[i:i + tt] for i in range(0, len(poe11), tt))

        elif session['yearop'] == "exyear":
            for r in range(0,len(subject_name)):
                direct=cur.execute("select * from direct where subject=%s and course=%s and exyear=%s and college=%s",(str(subject_name[r][4]),str(course),year,clg,))
                direct=cur.fetchall()
                directdata.append(direct)
                indirect=cur.execute("select * from indirect where subject=%s and course=%s and exyear=%s and college=%s",(str(subject_name[r][4]),str(course),year,clg,))
                indirect=cur.fetchall()
                indirectdata.append(indirect)
                # pe11=cur.execute("select po.val from po where subject='"+str(subjects[r][4])+"' and course='"+str(course)+"' and exyear=%s",(year,))
                # pe11=cur.fetchall()
                # poe11 = list((int(x[0])) for x in pe11)
                # resultTuple = list(poe11[i:i + tt] for i in range(0, len(poe11), tt))
        ...
        didata=[directdata,indirectdata]

        

        return render_template('copoattainment.html', all=allForCoAttainment,all2=allForPoAttainment,alldata=alldata,poval=poval,didata=didata)
    else:
        return redirect('/year')
...
@app.route('/addcollege',methods=["POST","GET"])
def addcollege():
    cursor = mysql.connection.cursor()
    if request.method=="POST":
        name=request.form["name"]
        ...
        cursor.execute("insert into subadmin(name,college,city,state,phnno,email,pass) values (%s,%s,%s,%s,%s,%s,%s)",(name,college,city,state,phn,email,passs))
        mysql.connection.commit()
        return redirect('/')
    return redirect('/')
...
def adminprofile():
    ...
    notverified=cursor.execute('select * from subadmin where verify !="YES"')
    notverified=cursor.fetchall()
    mysql.connection.commit()
    return render_template('admindashboard.html',verified=verified,notverified=notverified)
